...
- `data/merchable-items.ts` — typed reader for `merchableItems.json` (inlined at build time by esbuild). `getMerchableItems`, `getMerchableItem`, `isMerchable`, `getFirstUnoccupiedMerchableItem` (returns `BuyScanResult` with runtime quantity/profit/hr), `evaluateItemAtRuntime` (cash-stack-aware runtime evaluation), `MerchableItem`, `BuyScanResult`, `RuntimeEvaluation`.
