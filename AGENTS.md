...
The offer cache is stored in a visible string setting `offerCacheSetting` (key `offerCache`, default `'{}'`, `hidden: false`). The setting is visible in the plugin settings panel so you can manually copy the JSON out before closing the client and paste it back in after restarting to preserve offer metadata across client restarts. The cache is a JSON object keyed by in-game player name, each containing a per-account `OfferCacheData` map from item name to `OfferCacheEntry`. This mirrors the mixology bot's `humanizationState` persistence pattern.
...
- **Abort history**: Aborted offers (stale buys with 0% progress, partial-fill buys, sell aborts, frozen swap-outs) are recorded in a separate `abortHistorySetting` (visible, key `abortHistory`) via `recordAbort()` in `data/abort-history.ts`. Each entry captures: `item`, `type` (buy/sell), `requestedQty`, `filledQty` (what actually filled before the abort — 0 for no-fill buys), `reason` (stale reason string or 'frozen swap-out'), `elapsedMin`, `etaMin` (original cached ETA for comparing actual vs expected), `price`, and `date`. Capped at 200 entries per account. This is the key diagnostic for low overnight profit — aborted 0-fill buys represent wasted time and slot occupancy that merch history doesn't capture (they leave no completed cycle). The setting is visible for manual backup across client restarts. The abort history is dumped alongside merch history during breaks and via the "Log Merch & Abort History" button.
...
- `autoLoop.abortSlotInfo` — set when an abort flow is initiated (stale offer or frozen swap-out). Carries `{ type, itemName, progress, reason, etaMin, requestedQty, price, placedAt }` for abort history recording. Cleared (`null`) when the abort flow ends (done or failed) and in `resetAutoLoop()`. The extended fields (reason, etaMin, requestedQty, price, placedAt) are captured from the cache entry and GE slot at abort trigger time so the abort history entry can be recorded at abort completion.
