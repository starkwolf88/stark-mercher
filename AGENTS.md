...
- **`accountBreakStateSetting`** (hidden) — stores per-account break state as JSON: `{ accountName: { lastLogoutAtMs, minBreakDurationMs } }`. Used to determine if an account's minimum break has lapsed.
