...
- `input/typing-mistakes.ts` — `TypingMistakeProfile` (`nameMistakeChance`, `quantityMistakeChance`, `priceMistakeChance`), `typeStringWithMistake`, `setTypingMistakeProfileForAccount`, `cancelTypingMistakeSequence`; occasional wrong-character + backspace correction during GE typing. Includes a safety watchdog (`MAX_MISTAKE_DURATION_MS = 8000ms`) that force-clears a stuck sequence if the Titan runtime fails to fire a `setTimeout` callback (observed in production: the realisation delay's `setTimeout` never fired, leaving `mistakeSequenceActive=true` permanently and causing the buy flow to time out at the `waitForTyping` step). `BuyOfferFlow` and `SellOfferFlow` also call `cancelTypingMistakeSequence()` in `waitTick()` (on re-attempt) and `fail()` to prevent a stuck sequence from poisoning subsequent typing operations.
...
8. **Stale offers**: For each active slot, check stale conditions (sell: 75% ETA + <25% sold; buy: 125% ETA + 0 bought, or 90% ETA + <50% bought for multi-qty). Items removed from `merchableItems.json` are NOT immediately aborted — they may still be merchable, and the volatile list during development caused abort churn on every restart. Instead, ETA-based checks handle them using the cached `purchaseEtaMinutes` from buy time. Start an `AbortOfferFlow` if stale. Buy offers are frozen for 5 min when the abort is triggered — the freeze is NOT re-applied if the item is already frozen (e.g. a previous abort attempt failed and is being retried), and is NOT applied for config-driven aborts (item removed from list — re-buying is fine if it's added back). After the abort flow completes, the cache entry is only removed if the item is NOT in inventory — if the buy partially filled during the abort flow (especially after a failed first attempt), the collected items are in inventory and the cache entry is kept to preserve buy-limit tracking. The sell scan will pick up the items in the next iteration. An abort history entry is recorded at abort completion via `recordAbort()` in `data/abort-history.ts`, capturing item, type, requested/filled qty, reason, category (`'eta'`, `'swap'`, or `'config'`), elapsed vs ETA, price, and timestamp — this is the key diagnostic for low overnight profit (aborted 0-fill buys leave no trace in merch history).
9. **Selling**: Find empty slot + non-coin inventory item not being bought. Items being SOLD in another GE slot are NOT skipped — OSRS allows the same item to be listed in multiple sell slots simultaneously, so the bot will place a second sell offer for an item that already has one in another slot. Only items with an active BUY offer in a GE slot are skipped (to avoid selling partially-filled buy collections before the buy completes). Use cached sell price (revised if re-listing) or merchableItems.json price. Pass the actual sell quantity and the item's GE buy limit to `recordSellOffer()` for buy-limit tracking. Start a `SellOfferFlow`.
10. **Buying**: Find empty slot + first unoccupied merchable item that is affordable (full `totalPurchasePrice` ≤ coins) AND not buy-limited (within the 4-hour GE cooldown) AND not buy-frozen. Record buy offer in cache. Start a `BuyOfferFlow`. If no item is affordable at full quantity, a **partial-quantity fallback** scans for items where at least 1 unit is affordable and the expected profit (`reducedQty * profitMargin`) is ≥ 15k gp. The reduced quantity is `floor(coins / purchasePrice)` capped at the GE limit. This keeps empty slots productive when most of the cash stack is tied up in other offers. If no full or partial item is found, the diagnostic logs a summary: how many items were occupied, buy-limited, frozen, and unaffordable. **Frozen fallback**: If no non-frozen item is available (full or partial), the bot falls back to a frozen item rather than leaving a slot empty — an empty slot earns 0gp, while a frozen item might buy if market conditions have changed since the abort. Among frozen items, the one with the **soonest-expiring freeze** is preferred (frozen the longest ago → most time for conditions to change). This applies to both the full-quantity and partial-quantity fallbacks.
