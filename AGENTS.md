...
- `widgets/bot-overlay.ts` — HUD overlay panel. `renderBotOverlay` draws Status, Inventory Coins, Daily Profit, and TIMERS (Session, Next Account, Next Hop, Sleep/Wake Time). Registered via `this.overlay({ layer: 'AboveWidgets' })` in `stark-mercher.ts`. Expensive lookups (`isMerchableDataValid`, `getSoonestBreakEndMs`, `getNextAccountName`, `isRotationEnabled`) are cached with a 2-second TTL to avoid per-frame `JSON.parse` and item iteration lag at 60fps.
...
- **Short breaks** (1-10 min, ETA-based two-tier 50%/90%): Triggered when the auto-loop has nothing to do (all slots occupied, nothing to collect/sell/buy). The auto-loop sets `bot.loopIdleForBreak = true` when it reaches the idle branch. The break system in `breakStep()` checks these flags but enforces a **randomised tick-based delay** before actually logging out:
  ...
  **Break duration**: When the auto-loop goes idle, it computes `bot.nextActionEtaMin` — the minimum remaining time (in minutes) until the next action on any slot. For each active slot, this is the earlier of:
  ...
  - **Stale abort**: the offer hitting its ETA abort threshold (90% for multi-qty buys with <50% progress; min(125%, 15min absolute for non-lowball / 30min for lowball) for 0-fill buys; 50-95% for sells with <25% progress, scaled by profit margin via `computeSellEtaAbortRatio`; 100% otherwise)
  ...
  - The `checkedAtHalfEta` flag is set when a short break starts and cleared when any GE action happens (collect/sell/buy/abort — detected via the `wasIdle` check at the top of the auto-loop). It is also cleared in `resetInFlightActionState()` and the unexpected-logout handler.
  - Both tiers are clamped to a **randomized 1–2 min floor** and **10 min ceiling**. If no ETA data is available (`nextActionEtaMin <= 0`), falls back to `sampleShortBreakDuration()` (random 2-5 min + profile variance/long-tail).
...
When multi-account rotation is enabled and the bot is logged out (`breakPhase === 'logged_out'`), the overlay:

- **Status countdown** uses `getSoonestBreakEndMs()` — the minimum `breakEndMs` across all non-sleeping roster accounts — rather than the current account's `breakTargetEndMs`. This shows the actual wait time until the next account becomes eligible, which may be sooner than the current account's own break end.
- **Next Account** line shows the account that will log in next, via `getNextAccountName()`. If any account is eligible now, it shows the one with the oldest `lastLoginAtMs` (same as `selectNextAccount`). If none are eligible, it shows the one with the soonest `breakEndMs`. Green when eligible now, orange when still on break.

When rotation is disabled, the overlay uses `breakTargetEndMs` as before and the Next Account line is hidden.
...
When rotation is enabled (2+ accounts), the buy scan skips items that are already being merched (buy OR sell phase) by enough other accounts. This prevents multiple accounts from competing on price for the same item and avoids both accounts having capital tied up in the same slow item. The threshold is `min(2, rosterSize - 1)`: with a 2-account roster, the threshold is 1 (no overlap — if the other account is buying OR selling an item, skip it); with 3+ accounts, the threshold is 2 (up to 2 accounts can merch the same item, but the 3rd skips it). The check reads all accounts' offer caches from the shared `offerCacheSetting` (keyed by account name) via `getCrossAccountActiveItemCount()`, which returns a `Map<itemName, count>` of active `mode: 'buy'` OR `mode: 'sell'` entries across all other accounts. Items where the count meets the threshold are added to `crossAccountSkipNames` and passed to all buy scan functions (primary, frozen fallback, partial fallback, and swap candidate).

The dedup counts both buy and sell modes because with one-account-at-a-time rotation, account A may have already completed its buy and moved to sell mode by the time account B logs in. If the dedup only counted buy mode, B would buy the same item A is still selling — tying up both accounts' capital in the same item and creating sell-side price competition when B eventually lists its sell offer.
...
- **Status** — top-level action string from `bot.statusText`. Updated by the auto-loop at each transition (walking to GE, opening GE, collecting, checking stale offers, aborting, placing buy/sell offer, idle). Break phases are handled directly by the overlay's `getStatusText()` (checks `bot.breakPhase` → "Logging out for break...", "On break", "Sleeping", "Logging in..."). In Paused mode, `statusText` is set to `'Paused'`. When logged out with rotation enabled, shows the soonest break-end countdown across all accounts. When merchable data is invalid, shows `Data Invalid`.
- **Next Account** — shown in the TIMERS section when multi-account rotation is enabled and the bot is logged out. Displays the account name that will log in next (oldest `lastLoginAtMs` among eligible, or soonest break end if none eligible). Green when eligible now, orange when still on break.
