...
5. When the break timer expires (and no immediate or idle rotation happened), `selectNextAccount()` iterates the roster starting from `rotationIndex`:
...
6. The selected account's credentials are staged and the bot logs in. On successful login, the account's break state is cleared.
