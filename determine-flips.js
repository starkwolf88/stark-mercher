...
const MAX_RESULTS = 10; // Maximum number of merchable items to output after sorting by profitability
const GE_TAX_PERCENTAGE = 2; // Grand Exchange sale tax percentage deducted from sell price
const CASH_STACK_MILLIONS = 23; // Total flipping cash in millions for readability
...
const AVERAGE_SLOT_CASH_STACK_ALLOCATION_RATIO = 0.2; // Percentage of total cash assumed to be used per GE slot (~5 slots)
...
async function getTimeSeriesData() {
    ...
    for (const itemData of filteredItems) {
        ...
        if (!existingSeries || existingSeries.length === 0) {
            fetchNewData = true;
            console.log(`No time series data for '${itemData.itemName}' [${itemData.itemId}]. Fetching data from API.`);
        } else {
            ...
            if (hoursBehind === 1) {
                console.log(`[${expectedTimestampDate}] doesn't exist against '${itemData.itemName}' [${itemData.itemId}]. Adding data from 1h data.`);
            ...
            // 2 hours outdated. Fetch from API.
                ...
                fetchNewData = true;
                console.log(`Time series data for '${itemData.itemName}' [${itemData.itemId}] is at least 2 hours outdated. Fetching data from API.`);
            }
        }
...
const MARKET_SHARE_ASSUMPTION_PERCENTAGE = 35; // Assume 35% market share to estimate ETA's better
const MAX_TURNOVER_HOURS = 3; // Maximum allowed time for a full flip cycle (purchase + sale)
